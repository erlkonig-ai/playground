...
path is `ssh -6 jp@<jail-host>`: that hostname's IPv4 address is a different
...
1. **Use and re-audit the real administration path.** Force
   `ssh -6 jp@<jail-host>`; its IPv4 address is a different SSH host. Confirm
...
`<jail-host>` is intentionally absent from this list: there is currently no
