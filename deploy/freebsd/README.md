...
Runs the sandbox provider *on* the jail host (`<jail-host>`, FreeBSD
...
the operator sibling layout. The server profile skips the GUI/faculties
stack entirely:
...
rsync -a --delete \
  ...
  <jail-host>:playground-build/

# verify the pile rail held before anything else:
ssh <jail-host> "find playground-build -name '*.pile'"   # must print nothing
...
(`ssh -L 8377:127.0.0.1:8377 <jail-host>`) gives an operator with an
