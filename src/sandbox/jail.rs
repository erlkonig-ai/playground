...
//!
//! Drives a remote FreeBSD host (default `<jail-host>`) over SSH and maps
...
//!
//! `<jail-host>` is a **shared machine** (JP's coworker has access), so it
//! is not a trusted destination for Operator's private pile content — the
...
//!     `/shared` (same append-only semantics as self.pile — the workspace already
...
pub struct JailBackend {
    ...
    /// This is the COWORKER's server-side seed — NEVER Operator's private pile.
...
impl SandboxBackend for JailBackend {
    ...
    fn open_session(&self, spec: &SessionSpec) -> Result<SessionId> {
        ...
        // provision_sandbox), NOT the pile in `spec.tenant.pile`. Operator's
        ...
        eprintln!(
            "[{}] note: Operator's private pile '{}' stays on the local host; the \
    ...
    fn provision_sandbox(&self, spec: &SessionSpec) -> Result<()> {
        ...
        let provision = (|| -> Result<()> {
            ...
            // coworker's SERVER-BORN pile — distinct from Operator's private pile,
            ...
            // self.pile — the workspace already concurrently appends one pile).
...
mod tests {
    ...
    /// from the caller's (Operator-private) spec pile.
    #[test]
