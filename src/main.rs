...
struct McpArgs {
    ...
    #[arg(long, default_value = "<jail-host>")]
    jail_host: String,
...
/// local-model worker actually produced without a GUI dashboard.
...
fn create_thought_and_request(
    ...
    let context_json = if let Some(exec_result_id) = about_exec_result {
        ...
        *prev_cover = Some(MemoryCoverState {
        ...
        // Prepend the system prompt so the very first turn of a fresh worker has
